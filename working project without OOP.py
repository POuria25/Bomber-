...
def calculer_vitesse_acceleration(altitude_avion, temps_maintenant):
    ...
    v = - (altitude_avion - pprecedent) / (temps_maintenant - tprecedent)

    a = (v - vprecedent) / (temps_maintenant -  tprecedent)


    tprecedent = temps_maintenant
    pprecedent = altitude_avion
    vprecedent = v
...
def mrua_1d(depart, temps_depart, acceleration, temps_maintenant):
    ...
    p = depart + 1/2 * acceleration *((temps_maintenant-temps_depart)** 2)
...
def calculer_tir(altitude_larguage, altitude_cible, acceleration,
	...
	temps_vol = math.sqrt(2*(altitude_cible-altitude_larguage) / acceleration)
	temps_tir = prochain_temps_cible - temps_vol
	
	if (temps_tir > temps_maintenant):
	
		return (True, temps_tir)
...
image_nuage = pygame.image.load('cloud.png').convert_alpha(fenetre)

altitude_avion = dimensions_fenetre[1] / 2
image_avion = pygame.image.load('plane.png').convert_alpha(fenetre)
